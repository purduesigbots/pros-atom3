...
import { createTerminalTab, analytics } from './util'
import { conductNewProject, conductUpdateProject } from './conductor'
import { uploadPrompt } from './serial'
...
export default {
  'pros:new-project': {
    ...
    didDispatch: (event) => {
      analytics.action.post("New Project")
      conductNewProject()
    }
  },
  'pros:upgrade-project': {
    ...
    didDispatch: (event) => {
      analytics.action.post("Upgrade Project")
      conductUpdateProject()
    }
  },
  'pros:build': {
    ...
    didDispatch: (event) => {
      analytics.action.post("Build Project")
  ...
  'pros:upload': {
    ...
    didDispatch: (event) => {
      analytics.action.post("Upload Project")
  ...
  'pros:terminal': {
    ...
    didDispatch: (event) => {
      analytics.action.post("Open Terminal")
  ...
  'pros:quick-action': {
    ...
    didDispatch: async (event) => {
      analytics.action.post("Quick Action")
  ...
  'pros:upload-prompt': {
    displayName: 'PROS: Upload Prompt',
    didDispatch: (event) => {
      uploadPrompt();
    }
  },
  'pros:show-welcome': {
    displayName: 'PROS: Show Welcome Page',
    didDispatch: (event) => {
